...
if __name__ == '__main__':
    # 画像の読み込み
    img_src = cv2.imread("/home/makoto/OnClouds/robotsmanageserver/node_whill/pgmMAPDB/NIC/6F/NIC6F.pgm", 0)

    # 二値変換
    thresh = 100
    max_pixel = 255
    ret, img_dst = cv2.threshold(img_src,
                                 thresh,
                                 max_pixel,
                                 cv2.THRESH_BINARY)
    inv_img_dst = cv2.bitwise_not(img_dst)
    # 表示
    pts = edges2polylines(inv_img_dst)
    img = np.zeros((4000, 4000, 1), np.uint8)
    for pt in pts:
        hoge = np.array(pt, np.int32).reshape((-1, 1, 2))
        cv2.polylines(img,hoge,True,(255,255,255))
